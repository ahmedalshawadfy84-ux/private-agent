...
class ActionHandler {
  ...
  final ScreenProjectionService _projectionService = ScreenProjectionService();

  ShizukuService get shizuku => _shizuku;
  ScreenAutomationService get screenAutomation => _screenAutomation;
  ScreenProjectionService get projectionService => _projectionService;
  ...
  /// Cancel the currently running task. Also tears down the MediaProjection
  /// foreground service so the privacy indicator disappears immediately.
  Future<void> cancelTask() async {
    _currentExecutor?.cancel();
    try {
      await _projectionService.stop(reason: 'User pressed Stop');
    } catch (_) {
      // ignore — the executor also tries to stop on cancel
    }
  }

  /// Whether the MediaProjection foreground service is currently active.
  Future<bool> isProjectionRunning() => _projectionService.isRunning();
}
